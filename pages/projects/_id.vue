        ...
        <div class="w-1/4">
          ...
          >
            ...
            <div class="flex mt-3">
              <div class="w-1/4">
                <img
                  :src="$axios.defaults.baseURL + '/' + campaign.data.data.user.image_url "
                  alt=""
              ...
              <div class="w-3/4 ml-5 mt-1">
                <div class="font-semibold text-xl text-gray-800">
                  {{ campaign.data.data.user.name }}
                </div>
                <div class="font-light text-md text-gray-400">
                  {{ campaign.data.data.backer_count }} backer
                </div>
              </div>
            </div>
            ...
            <!-- check conditional if login show the button of fund -->
      ...
      <div class="flex justify-between items-center">
        <div class="w-full md:w-3/4 mr-6">
          ...
          <div class="relative progress-bar">
            ...
            >
              <div
                :style="'width: ' + (campaign.data.data.current_amount / campaign.data.data.goal_amount) * 100 + '%'"
                class="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-purple-progress progress-striped"
              ></div>
            </div>
          </div>
          <div class="flex progress-info mb-6">
            <div class="text-2xl">{{ (campaign.data.data.current_amount / campaign.data.data.goal_amount) * 100 }}%</div>
            <div class="ml-auto font-semibold text-2xl">Rp {{ Intl.NumberFormat('id-ID').format(campaign.data.data.goal_amount)}}</div>
          </div>
...
export default {
  ...
  mounted() {
    this.default_image = this.$axios.defaults.baseURL + '/' + this.campaign.data.data.image_url
  },
  methods: {
    changeImage(url){
  ...
  async asyncData({$axios, params}){
    ...
    console.log(campaign.data.data.user.name)
    return { campaign }
  }
}
</script>
